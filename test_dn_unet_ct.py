...
from utils.loss import dice_loss2
...
if __name__ == '__main__':
    ...
    for test_idx in range(num_domain):
      ...
      #  print("means_list", means_list, "vars_list", vars_list)
        model.eval()
        ...
        dataloader = DataLoader(dataset, batch_size=FLAGS.batch_size, shuffle=False, num_workers=4, pin_memory=True)
        ...
        with torch.no_grad():
            for idx, (batch, id) in enumerate(tbar):
                ...
                # Select best result
                for domain_id in range(1):
                    new_dis = cal_distance(means, means_list[domain_id], vars, vars_list[domain_id])
                    if new_dis < dis:
                        print(new_dis, dis, "domain_id:", domain_id) #, means, means_list[domain_id], vars, vars_list[domain_id])
                ...
                #total_dice = total_dice + 1 - dice_loss2(pred_y, mask)
                if pred_y.sum() == 0:
                    print("pred_y is 0")
                    total_hd += 100
                    total_asd += 100
