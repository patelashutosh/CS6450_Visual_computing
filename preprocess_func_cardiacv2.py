...
def main(data_root, modality, target_root, sub_modality):
    list_dir = os.listdir(data_root)
    tbar = tqdm(list_dir, ncols=70)
    count = 0
    for name in tbar:
        if 'label' in name:
            nib_mask = nib.load(os.path.join(data_root, name))
            img_name = name.replace("label", "image")
            nib_img = nib.load(os.path.join(data_root, img_name))
        else:
            continue
        ...
        for i in range(slices.shape[2]):
            ...
            Source-Similar
            """
            save_img(slices[:, :, i], masks[:, :, i], os.path.join(target_root, str(sub_modality), modality + '_ss', 'sample{}_0.npz'.format(count)))
            save_img(nonlinear_slices_2[:, :, i], masks[:, :, i], os.path.join(target_root, str(sub_modality), modality + '_ss', 'sample{}_1.npz'.format(count)))
            save_img(nonlinear_slices_4[:, :, i], masks[:, :, i], os.path.join(target_root, str(sub_modality), modality + '_ss', 'sample{}_2.npz'.format(count)))
            """
            Source-Dissimilar
            """
            save_img(nonlinear_slices_1[:, :, i], masks[:, :, i], os.path.join(target_root, str(sub_modality), modality + '_sd', 'sample{}_0.npz'.format(count)))
            save_img(nonlinear_slices_3[:, :, i], masks[:, :, i], os.path.join(target_root, str(sub_modality), modality + '_sd', 'sample{}_1.npz'.format(count)))
            save_img(nonlinear_slices_5[:, :, i], masks[:, :, i], os.path.join(target_root, str(sub_modality), modality + '_sd', 'sample{}_2.npz'.format(count)))
            count += 1
...
if __name__ == '__main__':
    ...
    modality = 'ct'
    sub_modality = 500
    main(data_root, modality, target_root, sub_modality)